...
import { FaSearch, FaFacebookSquare, FaTwitterSquare, FaLinkedin, FaChevronDown, FaTimes, FaBars, FaUser, FaHeart, FaShoppingCart, FaGift, FaPhone, FaHome, FaStore, FaEnvelope } from "react-icons/fa";
...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white text-black">
      ...
      <div className="bg-white border-b">
        <div className="max-w-[1200px] mx-auto px-4 lg:px-0">
          <div className="h-[60px] flex items-center justify-between">
            ...
            <button
              onClick={toggleMenu}
              className="min-[765px]:hidden text-black"
            >
      ...
      {/* Navigation Menu */}
      <div className="bg-pink-100  navbarMain  text-black max-[765px]:hidden">
        <div className="max-w-[1200px] mx-auto px-4 lg:px-0">
          <div className="h-12 flex items-center justify-between">
            {/* Menu Items */}
            <div className="flex items-center space-x-8 text-sm font-normal">
              <Link to="/HomePage" className={`hover:text-pink-600 ${isActive('/HomePage') ? 'text-gray-900' : ''}`}>
                HOME
              </Link>
              <Link to="/shop" className={`hover:text-pink-600 ${isActive('/shop') ? 'text-gray-900' : ''}`}>
                SHOP
              </Link>
              <Link to="/contact" className={`hover:text-pink-600 ${isActive('/contact') ? 'text-gray-900' : ''}`}>
                CONTACT
              </Link>
              <Link to="/OccasionsPage" className={`hover:text-pink-600 ${isActive('/OccasionsPage') ? 'text-gray-900' : ''}`}>
                OCCASIONS
              </Link>
              <Link to="/about" className={`hover:text-pink-600 ${isActive('/about') ? 'text-gray-900' : ''}`}>
                ABOUT
              </Link>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden ">
          ...
          <div className="fixed inset-y-0 left-0 max-w-xs w-full bg-pink-200 shadow-xl z-50 overflow-y-auto">
            <div className="p-6">
              <button onClick={toggleMenu} className="absolute top-5 right-5">
                <FaTimes className="w-6 h-6" />
              </button>
              <div className="mt-8">
                <Link to="/HomePage" className="text-2xl flex items-center text-center">
                  <span className="font-['Bodoni_MT'] font-bold text-xl">MERA Bestie</span>
                </Link>
                <Link to="/HomePage" className="block py-2.5 text-lg font-medium hover:text-pink-500" onClick={toggleMenu}>
                  <FaHome className="inline-block mr-2 text-pink-500 text-lg" /> Home
                </Link>
                <Link to="/shop" className="block py-2.5 text-lg font-medium hover:text-pink-500" onClick={toggleMenu}>
                  <FaStore className="inline-block mr-2 text-pink-500 text-lg" /> Shop
                </Link>
                <Link to="/contact" className="block py-2.5 text-lg font-medium hover:text-pink-500" onClick={toggleMenu}>
                  <FaEnvelope className="inline-block mr-2 text-pink-500 text-lg" /> Contact
                </Link>
                <hr className="my-4" />
                <Link to="/OccasionsPage" className="block py-2.5 text-lg font-medium hover:text-pink-500" onClick={toggleMenu}>
                  <MdCelebration className="inline-block mr-2 text-pink-500 text-lg" /> Ocassion
                </Link>
                <Link to="/about" className="block py-2.5 text-lg font-medium hover:text-pink-500" onClick={toggleMenu}>
                  <FcAbout className="inline-block mr-2 text-pink-500 text-lg" /> About
                </Link>
               
                <div className="mt-4 flex items-center">
                  <FaPhone className="w-4 h-4 mr-2 text-green-700" />
                  <span>(219) 555-0114</span>
                </div>
                <hr className="my-4" />
                <div className="flex items-center gap-3 socialLink">
                  <FaFacebookSquare className="slink text-blue-600 text-lg" />
                  <FaTwitterSquare className="slink text-sky-800 text-lg" />
                  <FaLinkedin className="slink text-sky-500 text-lg" />
                </div>
              </div>
            </div>
          </div>
