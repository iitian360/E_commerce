...
const FetchComplain = () => {
  ...
  const sortedComplains = React.useMemo(() => {
    if (!Array.isArray(complains)) return [];

    let sortableComplains = [...complains];
    if (sortConfig.key !== null) {
      sortableComplains.sort((a, b) => {
        if (a[sortConfig.key] < b[sortConfig.key]) {
          return sortConfig.direction === 'ascending' ? -1 : 1;
        }
        if (a[sortConfig.key] > b[sortConfig.key]) {
    ...
    return sortableComplains;
  }, [complains, sortConfig]);

  const filteredComplains = sortedComplains.filter(complain => 
    complain.complaintNumber?.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <div className="flex">
      <Helmet>
        <title>Complains | Admin | Mera Bestie</title>
      </Helmet>
      <Sidebar />
      <div className="flex-1 p-8 ml-[5rem] lg:ml-64 bg-pink-50 min-h-screen">
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-10 w-full max-w-2xl px-4">
          ...
          <table className="min-w-full table-auto">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredComplains.map((complain) => (
                <tr key={complain.complaintNumber}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${complain.status === 'Resolved' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <select
                      value={complain.status || 'Pending'}
                      onChange={(e) => handleStatusChange(complain.complaintNumber, e.target.value)}
                      className="border rounded px-2 py-1 text-sm"
                    >
                      <option value="Pending">Pending</option>
                      <option value="Resolved">Resolved</option>
