...
const Shop = () => {
  ...
  return (  
    ...
    <div className="bg-pink-100">
      ...
      >
        <h2 className="text-4xl font-bold text-black mt-24">Shop by category</h2>
      ...
      <div className="max-w-7xl mx-auto">
        <h3 className="text-3xl font-bold mb-4 text-center">Select category</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {categories.map((category, index) => (
            <div
              key={index}
              className="bg-white shadow-2xl overflow-hidden cursor-pointer border-2 border-pink-400"
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-4 items-center">
            <button className="bg-pink-300 px-4 py-2 rounded filterBtn" onClick={() => filterProducts('all')}>All Products</button>
            <div>
              <label htmlFor="sort" className="mr-2">Sort by</label>
              <select id="sort" className="px-2 py-1 rounded bg-pink-200" onChange={(e) => sortProducts(e.target.value)}>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`p-2 border rounded ${viewMode === 'grid' ? 'bg-pink-300' : ''}`}
              >
                ▤
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 border rounded ${viewMode === 'list' ? 'bg-pink-300' : ''}`}
              >
                ☰
              </button>
            </div>
